...
> - 📊 **Pitch deck:** [Google Drive (PDF)](https://drive.google.com/file/d/1smgC3O0UCqabXOB95sfdtJQHHvsQv5UG/view?usp=sharing) · [in-repo copy](docs/VerdictAML-finals.pdf)
> - 🌐 **Live console (Render):** https://aml-alert-triage-copilot-1.onrender.com
> - 🔌 **Live API — verify the numbers yourself:** https://aml-alert-triage-copilot.onrender.com — open [`/metrics`](https://aml-alert-triage-copilot.onrender.com/metrics) for the raw held-out numbers, [`/docs`](https://aml-alert-triage-copilot.onrender.com/docs) for the full API, [`/health`](https://aml-alert-triage-copilot.onrender.com/health) for liveness.
> - 📺 **7-min demo video (YouTube):** https://youtu.be/MAnAI7-EvsQ
>
...
## Key Features
* **Autonomous Queue Triage — with a human gate**: Squarely targeting the hackathon's *Autonomous AI Workforce* theme, the **Queue Agent** works the alert backlog unattended: it runs each alert through the triage→verifier pipeline and applies a **deterministic, audited Auto-Clear Policy** that auto-dismisses *only* high-confidence, verifier-agreed benign alerts — **43% of the held-out queue, auto-cleared unattended** (measured on SAML-D; the full `autoClearedShare` / `autoClearPrecision` breakdown is served live on `/metrics`). It **never auto-escalates or auto-files** — every STR-bound alert routes to a human `needsReview` inbox, and every autonomous clear lands in the audit trail. The analyst opens to a precomputed **Shift Briefing** and an **Auto-Cleared** QA lane to sample what the agent handled (ADR-0010). Autonomy on the dismiss side, a hard human gate on the escalate side.
...
* **Governed Self-Learning Suppression (closed loop)**: When an analyst dismisses a benign alert, the system learns a *bounded signature* from that decision and safely auto-suppresses future look-alikes — surfaced in a **Learned Patterns** tab. This is *not* silent model retraining: suppression is gated by verifier agreement, deterministic policy, and a **measured leakage/coverage frontier** (a pre-registered operating point cleared 80 of 250 held-out alerts with **0 observed leaks**, 3.75% upper bound), with cleared alerts kept inspectable via QA sampling (ADR-0021). This analyst-taught, firewall-gated control loop — not "two agents" — is the core innovation.
...
* **Slate & Mint (Cyber-Defense) Console**: A modern, clean, dark-themed interface built specifically for security and financial audit contexts, organized into five tabs — **Alert Queue**, **Learned Patterns**, **System Performance**, **Governance** (validation dossier, QA sampling, leakage/coverage frontier, model-risk change control, readiness), and **Audit Trail**. Includes left-border highlighting of cited transactions and adversarial warning banners.
...
* **Offline Evaluation Suite**: Runs the live triage agent over a **held-out** sample of real SAML-D alerts (Oztas et al., 2023; frozen before any tuning) and reports the full picture — accuracy, recall, precision, per-typology recall, and a confusion matrix (ADR-0004). Measured: **recall 0.69**, **precision 0.73**, **accuracy 0.66** against a 0.40 baseline (per-typology recall: PT-01 0.80, ST-01 0.79, FI-01 0.70), all served live at [`/metrics`](https://aml-alert-triage-copilot.onrender.com/metrics) — open it to read the raw JSON these numbers come from.
...
- **The build calls the hosted DeepSeek API** so the agent reasoning is real and inspectable (the public Render instance runs **keyless**, so live `/triage` gracefully falls back to the precomputed result). The LLM is the *only* component that talks to an external service, and it sits behind a single swappable client (`backend/llm.py`): provider, base URL, and model are three environment variables. Because the client speaks the OpenAI-compatible protocol, pointing the system at a **self-hosted open-weight model** (Qwen, Llama, or DeepSeek weights served via vLLM/Ollama on the bank's own hardware) is a **configuration change, not a code change**. Data residency is the buyer's decision, not a constraint baked into the stack.
...
The core product described above is built and deployed. The roadmap below covers production-hardening and expansion beyond the current build.
...
### Differentiated Feature — Mule-Network Investigation (shipped, qualitative)

A shipped, differentiated investigative view. On its own, single-alert triage reasons about **one alert at a time** — exactly how a front-line analyst (and a rule-based monitoring system) sees the world, and exactly why **network-distributed laundering** can slip through. Mule-Network Investigation lifts the copilot from the single account to the *cluster*.
...
- **A governed, self-learning control loop** — the system learns a bounded suppression signature from an analyst's own dismissal and auto-clears future look-alikes, but only inside verifier agreement, deterministic policy gates, a measured leakage/coverage frontier, and QA sampling (ADR-0021). Analyst-taught, firewall-gated suppression with measured leakage — a closed-loop control system, not silent model retraining or a blind whitelist.
